...
https://www.youtube.com/watch?v=ptYUCjfNhJY&list=PLiQ766zSC5jMZgWWdqy_6TpLivRGQaFD-&index=19
"""
import sys
...
class MinHeap:
    ...
    assumes that the subtree are already heapify 
    """
    ...
    def min_heapify(self, i):
        ...
        _r = right(i)
        smallest = i
        if _l < self.heap_size and self.heap_arr[_l].element < self.heap_arr[i].element:
            smallest = _l
        if _r < self.heap_size and self.heap_arr[_r].element < self.heap_arr[smallest].element:
            smallest = _r
        if smallest != i:
...
if __name__ == '__main__':
    ...
    merge_k_sorted_arrays(test_array, len(test_array))
